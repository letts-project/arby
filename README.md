...
arby honors a per-dugdale `proxy:` (SOCKS5 or HTTP) declared in `letts.yaml` —
every fan-out and stream to that dugdale tunnels through it (`proxy: null` on a
dugdale opts it out of a template-inherited proxy). Pass `--ignore-proxy` to
ignore those directives entirely and connect to dugdales directly.
